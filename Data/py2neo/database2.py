...
def database2():
    ...
    csvFile = open("my_python/plane_data_2.csv", "r")
    ...
    for item in reader:
        ...
        if item[0] != "Aircraft Type":
            ...
            if tar:
                plane_manufactured_by = Relationship(node_plane, "Manufactured_by", tar)
                my_graph.create(plane_manufactured_by)
            else:
                node_manufactor = Node("Manufactor", name=item[1])
                my_graph.create(node_manufactor)
                plane_manufactured_by = Relationship(
                    node_plane, "Manufactured_by", node_manufactor
                )
                my_graph.create(plane_manufactured_by)
            ...
            if tar2:
                ...
                plane_belong_to = Relationship(node_plane, "Belong_to", node_airline)
                my_graph.create(plane_belong_to)
