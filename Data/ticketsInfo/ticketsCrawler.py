...
def crawler(dcId,acId,sDate):
	...
	file.close()
	i=0
	file = open('cities/'+cityName[dcId]+'.txt','a',encoding='utf-8')
...
def start():
	depCityId=0
	arrCityId=0
	searchDate="2019-04-08"
	getCities()
	while depCityId<len(cityAb) and depCityId<285:
		file = open('cities/'+cityName[depCityId]+'.txt','w',encoding='utf-8')
		file.close()
		time.sleep(5)
		while arrCityId<len(cityAb) and arrCityId<285:
			if arrCityId!=depCityId:
				print('出发地:'+cityName[depCityId]+'  目的地:'+cityName[arrCityId])
				crawler(depCityId,arrCityId,searchDate)
			arrCityId+=1
		depCityId+=1
		arrCityId=0
start()
